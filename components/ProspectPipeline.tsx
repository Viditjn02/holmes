...
function FitRing({ score }: { score?: number }) {
  ...
  return (
    <div
      ...
      style={{ background: `conic-gradient(${hex} ${deg}deg, rgba(255,255,255,0.08) ${deg}deg)` }}
      title={`Fit ${clamped}/100`}
    >
      <div className="grid h-7 w-7 place-items-center rounded-full bg-panel">
...
function ProspectCard({ p }: { p: ProspectDoc }) {
  ...
  return (
    <div className="rounded-xl border border-line bg-panel/80 p-3 transition-colors hover:border-white/15 animate-row-in">
      <div className="flex items-start gap-2.5">
        <FitRing score={p.fitScore} />
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-semibold text-zinc-100">{p.company}</p>
          {(p.name || p.title) && (
      ...
      {p.email && (
        <div className="mt-2 flex items-center gap-1.5">
          <span className="truncate text-[11px] text-zinc-400">{p.email}</span>
          {p.emailVerified ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-good/15 px-1.5 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide text-good ring-1 ring-good/30">
            ...
            <span className="rounded-full bg-white/5 px-1.5 py-0.5 text-[9.5px] font-medium uppercase tracking-wide text-white/40">
              unverified
            </span>
          )}
        </div>
      ...
      {sig && sigMeta && (
        <div
          className="mt-2 flex items-start gap-1.5 rounded-lg border px-2 py-1.5"
          style={tintStyle(sigMeta.hex)}
        >
          <span className="mt-0.5 text-[9px] font-bold uppercase tracking-wide">{sigMeta.label}</span>
          <span className="line-clamp-2 text-[11px] leading-snug opacity-90">{sig.summary}</span>
        </div>
      )}
      ...
      <div className="mt-2 flex items-center justify-between text-[10.5px] text-zinc-500">
        ...
        {p.linkedinUrl ? (
          <a href={p.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-sky-400 hover:underline">
            in/
          </a>
        ) : (
          <span className="text-zinc-600">{initials(p.name, p.company)}</span>
        )}
      </div>
    </div>
  );
...
function Column({ stage, items }: { stage: ProspectStage; items: ProspectDoc[] }) {
  ...
  return (
    <div className="flex w-60 shrink-0 flex-col rounded-2xl border border-line bg-ink/40">
      <div className="flex items-center justify-between gap-2 border-b border-line px-3 py-2.5">
        ...
        <span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] font-medium tabular-nums text-zinc-400">
          {items.length}
        </span>
      </div>
      <div className="col-scroll col-fade flex max-h-[460px] min-h-[80px] flex-col gap-2 overflow-y-auto p-2">
        {items.length === 0 ? (
          <p className="px-1 py-3 text-center text-[11px] text-zinc-600">{meta.blurb}</p>
        ) : (
    ...
    return (
      <div className="flex gap-3 overflow-x-auto pb-2">
        {PIPELINE_STAGES.map((s) => (
          <div key={s} className="h-64 w-60 shrink-0 animate-pulse rounded-2xl border border-line bg-panel/50" />
        ))}
      </div>
    );
  }
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-end justify-between gap-3">
        <div>
          <h3 className="text-[15px] font-semibold text-zinc-50">Outbound pipeline</h3>
