              ...
              return (
                <section key={g.type}>
                  <div className="mb-2.5 flex items-baseline gap-2.5">
                    <h3 className="caption text-ink">
                      {g.label}
                    </h3>
                    <span className="text-[11px] text-ink/40">{g.blurb}</span>
                    <span className="text-[11px] text-ink/40">· {list.length}</span>
                  </div>
...
function BrainHeader({ pages, facts, runs }: { pages: number; facts: number; runs: number }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-hairline bg-canvas px-5 py-4">
...
function Stat({ value, label, accent }: { value: number; label: string; accent?: boolean }) {
  return (
    <div className="text-right">
      <p
        className={cn(
          "text-xl font-fig-headline tabular-nums leading-none text-ink",
          !accent && "text-ink/80",
        )}
      >
...
function BrainPageCard({ page, highlighted }: { page: KnowledgePageDoc; highlighted: boolean }) {
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-lg border bg-canvas p-4 transition-colors",
        fresh
          ? "border-success/40"
          : highlighted
            ? "border-ink/40"
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-[13.5px] font-fig-headline text-ink">{page.title}</p>
          <p className="truncate text-[11px] text-ink/40">{page.entityKey}</p>
        </div>
        {fresh && (
          <span className="caption inline-flex shrink-0 items-center gap-1.5 rounded-full bg-block-mint px-2 py-0.5 text-ink">
      ...
      <div className="mt-3 flex items-baseline gap-2">
        <span className="text-2xl font-fig-card tabular-nums leading-none text-ink">
...
function FactRow({ fact }: { fact: KnowledgeFactDoc }) {
  return (
    <li className="flex items-start gap-2 text-[11.5px] leading-snug">
      <span className="caption mt-0.5 shrink-0 rounded border border-hairline bg-surface-soft px-1.5 py-px text-ink/50">
        {fact.kind || "note"}
      </span>
...
function BrainSkeleton() {
  return (
    <div className="mt-5 grid gap-3 sm:grid-cols-2">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="h-36 animate-pulse rounded-lg border border-hairline bg-surface-soft" />
      ))}
    </div>
  );
}
...
function BrainEmpty() {
  return (
    <div className="mt-10 grid place-items-center px-6 py-12 text-center">
      <span className="flex h-12 w-12 items-center justify-center rounded-lg border border-hairline bg-surface-soft text-2xl">
        🧠
      </span>
      <h3 className="mt-4 text-[15px] font-fig-headline text-ink">
        The brain is empty — for now.
      </h3>
      <p className="mt-1.5 max-w-sm text-[12.5px] leading-relaxed text-ink/50">
